...
public class FakeDataPreload {
   ...
   public void preloadTestData() {
       ...
       * of the type ClickerRegistration, then it runs the dao save method on them:
       * dao.findById(ClickerRegistration.class, item1.getId());
   ...
   public void reloadTestData() {
      /*
       * This iterates over the fields in FakeDataPreload and finds all the ones which are
       * of the type ClickerRegistration, then it sets the field equal to the method:
       * dao.findById(ClickerRegistration.class, item1.getId());
       * This does the same thing as writing a bunch of these:
       * item1 = (ClickerRegistration) dao.findById(ClickerRegistration.class, item1.getId());
       */
      Field[] fields = this.getClass().getDeclaredFields();
      for (Field field : fields) {
         if (field.getType().equals(ClickerRegistration.class)) {
